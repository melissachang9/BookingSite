...
import {
  deleteManageBookingFormAttachmentAction,
  saveManageBookingFormProgressAction,
  submitManageBookingFormResponseAction,
  uploadManageBookingFormAttachmentAction,
} from "./actions";
import {
...
export function ManageBookingFormRuntime({
  token,
  requirement,
  initialAnswers,
  initialSavedAt,
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      ...
      const result = await saveManageBookingFormProgressAction({
        token,
      ...
      if (!result.ok) {
        ...
        setDraftSaveError(result.error ?? "Could not save your progress");
        return;
      }
  ...
  }, [answers, pending, requirement.id, token]);
  ...
  function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await submitManageBookingFormResponseAction({
        token,
        requirementId: requirement.id,
        uploadSessionId: requirement.id,
      ...
      <div className="flex flex-col gap-4 border-b border-stone-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div className="max-w-2xl">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-stone-500">
            {totalPending > 1 ? `${totalPending} forms pending` : "Pending form"}
          </p>
          ...
          <p className="mt-2 text-sm leading-6 text-stone-600 sm:text-base">
            {requirement.formDescription?.trim()
              ...
              : "Complete this form from your secure manage link so the business has the details it needs."}
          </p>
        </div>
        <div className="rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm text-stone-600 shadow-sm">
          <p>{totalPending > 1 ? `${totalPending} forms still pending` : "Submit once to finish this form"}</p>
...
function FileUploadField({
  ...
  async function handleFiles(files: File[]) {
    ...
    for (const file of files) {
      ...
      fd.append("uploadSessionId", requirementId);
      fd.append("fieldId", field.id);
      fd.append("kind", "file");
      fd.append("file", file);
      const res = await uploadManageBookingFormAttachmentAction(fd);
  ...
  async function removeAttachment(attachmentId: string) {
    ...
    fd.append("token", token);
    fd.append("uploadSessionId", requirementId);
    const res = await deleteManageBookingFormAttachmentAction(fd);
    if (!res.ok) {
        ...
        {locked ? (
          <>
            ...
            <button
              ...
              disabled={uploading}
              className="rounded-full border border-stone-300 px-3 py-1 font-semibold uppercase tracking-[0.16em] text-stone-700 hover:bg-stone-50 disabled:opacity-50"
            >
              Clear and redraw
            </button>
          </>
          ...
          <>
            <button
              ...
              className="rounded-full bg-stone-900 px-4 py-2 font-semibold uppercase tracking-[0.16em] text-white hover:bg-stone-800 disabled:opacity-50"
            >
            ...
            <button
              type="button"
              onClick={() => clearSavedSignature()}
              className="rounded-full border border-stone-300 px-4 py-2 font-semibold uppercase tracking-[0.16em] text-stone-700 hover:bg-stone-50"
            >
              Clear
            </button>
          </>
